...
MinIO/AIStor에 등록·attach 되어 있는 정책의 **사본**을 AccessLens가 들고,
그에 맞춰 로그인한 사용자에게 버킷별 권한을 설명한다. AccessLens는 권한을
바꾸지 않는다 — 권한 변경 도구와 권한 조회 도구는 같은 것이 아니어야 한다.

```
MinIO/AIStor  ──(sync-from-minio.sh)──▶  policy/generated/*.json     무엇을 허용하는가
   (진실)                                 policy/attachments.yaml    누가 들고 있는가
                                                    │ (ConfigMap)
                                                    ▼  대시보드 S3 IAM 카드
```

### S3 IAM 카드에 버킷별 권한 내역

- 정책 문서와 attach 사본을 함께 읽어 버킷마다 **무엇을 할 수 있는지**와 그
  권한이 **어느 정책에서 왔는지**를 표시. 액션 이름 대신 사람이 읽는 능력
  7종(목록/읽기/수명주기 조회/쓰기/수명주기 설정/삭제/버킷 정책)으로 접는다.
- 삭제와 "수명주기 설정"은 붉게 구분. ILM 만료 규칙은 `DeleteObject`와 경로만
  다를 뿐 데이터가 사라지는 결과가 같다.
- 정책 개수·attach 건수·로드 시각을 항상 함께 표시 — 이 값은 MinIO에 질의한
  것이 아니라 사본에서 계산한 것이라 실시간처럼 보이면 안 된다.
- 접을 수 없는 정책 내용(Deny, NotAction, Condition)은 조용히 버리지 않고
  경고로 올린다. 각 경고는 **어느 방향으로 틀렸는지**까지 말한다.
- 버킷 목록도 사본이 있으면 정책의 Resource ARN에서 유도 — 손으로 유지하던
  `s3iam.bucketMap`보다 어긋날 여지가 적다(없으면 기존대로 fallback).

### attach를 이름 추론이 아니라 사본으로

- 이전에는 그룹 CN에서 정책명을 지어냈다(`{bi, dev}` → `"bi-dev"`). 누군가
  이름을 손으로 맞춰뒀기 때문에만 맞는 답이었고, 한 그룹이 정책 두 개를
  드는 경우·사용자 DN 직접 attach·명명 규칙을 벗어난 그룹을 표현하지
  못했으며, 무엇보다 MinIO 실물과 대조할 방법이 없었다.
- `policy/attachments.yaml`이 MinIO가 자기 상태를 보고하는 것과 같은 모양으로
  그 정보를 담는다. DN 기준이며 대소문자·콤마 뒤 공백을 정규화해 비교한다 —
  매칭에 실패한 DN은 화면에서 "권한 없음"과 구분되지 않는다.
- `model.User`에 `DN`/`GroupDNs` 추가, 세션 토큰이 이를 나른다. 필드가 5개인
  옛 토큰은 거부한다(재로그인 필요) — 빈 DN으로 받아주면 "권한이 아무 데도
  없음"으로 조용히 해석되어 진짜 답과 구분되지 않는다.

### 선언 대조 (드리프트 검사) — `GET /api/s3iam/drift`, adm 전용

- 사본이 아직 유효한지 MinIO 실물과 비교한다. **선언에 없음**(백엔드에 있는데
  사본에 없음 — 사본만 보면 영원히 안 보이는 권한)과 **미적용**(사본에 있는데
  백엔드에 없음 — 화면이 없는 권한을 약속)을 구분한다.
- 필요한 admin 액션은 `admin:ListUsers`와 `admin:GetPolicy` **둘뿐**이며 이
  조합으로는 객체 데이터를 읽을 수 없다(실측 확인). 사용자 자격증명은 필요
  없다.
- 검사 실패는 "일치"로 접지 않고 그대로 올린다.
- 라이브 admin 호출이라 대시보드 로드마다 돌지 않고 버튼으로 실행한다.
- cluster-mesh2 첫 대조에서 아무도 선언하지 않은 attach 3건을 찾았다.
  `cn=adm`/`cn=dev`에 붙어 있던 `readwrite`(전 버킷 전권)는 제거 — 차트의
  `ldap.groupRoleMap` 기본값이 정확히 `adm`/`dev`/`view`라, 기본값대로 배포한
  사람이 그 이름의 그룹을 만드는 순간 구성원 전원이 전권을 얻는 함정이었다.

### 정책 티어 수정

- `PutLifecycleConfiguration`을 dev → adm으로 이동. dev는 `DeleteObject`가
  없어 "삭제 불가"를 표방했지만, ILM 만료 규칙으로 사실상 삭제가 가능했다.
  `GetLifecycleConfiguration`은 dev에 남긴다(읽기 전용).
- **MinIO에도 재적용이 필요하다** — 사본만 바꾸면 화면과 실제가 어긋난다.

### 그 외

- `examples/ldap-verify/` 신설 — 검증 스택 구성을 레포로. 이 구성이 살아있는
  클러스터 상태로만 존재해서 두 번 대가를 치렀다(노드 교체로 죽은 LDAP 주소를
  MinIO/Trino가 몇 주간 바라봄, PVC 없는 openldap의 빈 디렉토리).
- 대시보드 연결 다이어그램에서 위성 상태 점 제거 — 연결 여부는 링 색이 이미
  표현하고 있었다.
- `vault/` 디렉토리를 버전관리에서 해제(파일은 디스크에 유지). 실제 업무
  노트가 데모 데이터로 들어가 있었다.

### 시도했다가 철회한 것

사용자 본인 세션으로 실제 S3 호출을 해 권한을 검증하는 "프로브"를 만들었다가
되돌렸다. 배포해 보니 세 계정이 전부 같은 답을 받았는데, 프로브에 넘긴 세션이
로그인한 사용자가 아니라 연결 확인용 고정 서비스 계정의 것이었기 때문이다.
제대로 하려면 로그인 시점에 사용자의 STS 세션을 받아 서버에 보관해야 하고
(비밀번호를 보관하지 않으므로 다른 방법이 없다), 그러면 무상태 구조가 깨지고
프로세스가 사용자 자격증명을 들게 된다. 대조는 같은 질문에 사용자 자격증명
없이 답한다.
