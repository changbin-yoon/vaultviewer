...
- **LDAP 카드 신규.** 나머지 세 카드의 권한이 전부 여기서 나온 그룹 소속으로
  결정되므로 첫 자리에 둔다. 역할·소속과 함께 **어느 그룹에 속해 있고 그
  그룹이 무엇을 주는지**를 보여준다. 팀 규칙(`<팀>-<역할>`)에 맞는 그룹은 그
  의미를, 맞지 않는 그룹도 감추지 않고 그대로 보여준다.
  - `/api/me`와 `/api/login`이 `groups`(그룹 CN)를 함께 반환. 세션은 이미
    그룹 DN을 나르고 있으므로(MinIO가 attach를 DN으로 걸기 때문) 거기서
    CN을 유도한다 — 같은 것의 두 표기를 나르려고 토큰을 또 넓히지 않는다.
- **Vault 카드**를 권한 카드 형태로 정리. S3 IAM과 같은 능력 칩 문법으로
  읽기/생성/수정/삭제를 보여주고, 역할에 없는 능력은 "없음"으로 함께
  표시한다. 삭제는 붉게 구분.
- **OPA 카드는 뺀다(추후 추가).** 백엔드(`/api/opa`, `internal/opa`)는 그대로
  살아 있고 카드만 없다. "연동 예정" 목록에 남겨 그 사실이 화면에 드러난다.
- **계정 연결 구조도도 같이 맞춘다.** 위성 노드에서 OPA를 빼고 LDAP을 넣어,
  위성 넷이 권한 카드 넷과 정확히 1:1이 되게 했다. LDAP 위성의 부제는 다른
  위성이 역할을 보여주는 자리에 그 역할의 출처인 **그룹 수**를 보여준다.
  중심 노드는 계정 자체를 나타내며 부제를 (LDAP → 역할)로 바꿨다.
- 대시보드 소개 문구를 "이 계정의 역할이 … 권한을 결정합니다"에서 **"LDAP
  그룹 소속이 Trino·S3 IAM·Vault 권한을 결정합니다"**로 수정. LDAP은 역할을
  주는 쪽이라 이전 문장은 앞뒤가 맞지 않았다.
