...
**둘 다 MinIO에 실제로 등록·attach 되어 있는 것의 사본입니다.** AccessLens는
권한을 바꾸지 않고 설명만 합니다. 사본은 MinIO에서 내려받습니다:

```sh
ALIAS=<mc alias> sh examples/ldap-verify/sync-from-minio.sh
```
...
곳에 있으면 반드시 갈라지기 때문입니다. 비워두면 권한 내역만 빠지고 카드의
...
ConfigMap의 사본은 내려받은 시점의 것입니다. 그 뒤 MinIO에서 정책이 바뀌면
화면이 낡은 것을 보여주게 됩니다. `s3iam.drift`에 자격증명을 주면
**관리자 화면에서 사본이 아직 유효한지 대조**할 수 있습니다:
...
- **선언에 없음** — 백엔드에 있는데 사본에 없습니다. 사본만 보면 영원히
  안 보이는 권한이 실제로 부여돼 있다는 뜻이라 더 위험합니다.
- **미적용** — 사본에 있는데 백엔드에 없습니다. 화면이 실제로는 없는 권한을
  약속하고 있다는 뜻입니다.

차이가 나오면 사본을 다시 받으면 됩니다(`sync-from-minio.sh`) — 그래도 차이가
남으면 그건 MinIO 쪽에서 판단할 문제입니다.
